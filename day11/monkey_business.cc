...
struct Monkey {
  ...
  void IncreaseWorry() {
    if (increase_by == "old") {
      ...
      items[0] = worry_op(items.front(), stoi(increase_by));
    }
  }
...
void TakeTurn(Monkey &monkey, Monkey monkeys[], int lcm) {
  monkey.inspected += monkey.items.size();
  while (!monkey.items.empty()) {
    monkey.IncreaseWorry();
    monkey.DecreaseWorry();  // For part 1
...
int main() {
  ...
  while (getline(monkey_info, line, ':')) {
    ...
    if (line.find("Starting items") != string::npos) {
      ...
      string item;
      while (info >> item) {
        int worry_lvl = stoi(item.substr(0, item.find(",")));
      ...
      if (op == '+') {
        monkeys[curr_monkey].worry_op = plus<int>();
      } else {
        monkeys[curr_monkey].worry_op = multiplies<int>();
      }
